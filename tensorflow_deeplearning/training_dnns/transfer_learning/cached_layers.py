...
# Output of the last frozen layer
...
with tf.Session() as sess:
    init.run()
    restore_saver.restore(sess, "./dl_best_mnist_model_0_to_4")
    ...
    for epoch in range(n_epochs):
        ...
        for rnd_indices in np.array_split(rnd_idx,
            ...
            # Add cached layers
            h5_batch, y_batch = hidden5_train[rnd_indices], y_train2[
                rnd_indices]
            sess.run(
                training_op, feed_dict={
                    hidden5_out: h5_batch,
                    y: y_batch
                })
        loss_val, acc_val = sess.run(
            [loss, accuracy],
            feed_dict={
                hidden5_out: hidden5_valid,
                y: y_valid2
            })
        if loss_val < best_loss:
            save_patch = five_frozen_saver.save(
                sess, "./dl_mnist_model_5_to_9_five_frozen")
            best_loss = loss_val
            checks_without_progress = 0
...
with tf.Session() as sess:
    five_frozen_saver.restore(sess, "./dl_mnist_model_5_to_9_five_frozen")
